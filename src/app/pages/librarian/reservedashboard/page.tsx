...
const variants = {
  ...
  useEffect(() => {
   ...
   const deleteReserve = (id:number) =>{
    axios.delete(`http://localhost:8080/api/reserve/${id}`).then(() =>{
      setreserve(reserve.filter(reserves => reserves.id !== id))
         Swal.fire({
  ...
  return (
    ...
    <div className='logo'><span style={{fontSize:50,color:'orange', fontWeight:'bold'}}>L</span><span style={{fontSize:40,color:'white', fontWeight:'bold'}}>i</span><span style={{fontSize:35,color:'orange', fontWeight:'bold'}}>b</span><span style={{fontSize:25, color:'white', fontWeight:'bold'}}>rary</span></div>
      ...
      transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }}>
              <h1 style={{marginTop:30}} className='text-center text-3xl text-blue-950 font-bold'>The Reservation Interface</h1>
              <br />
              <button><Link style={{textDecoration:'none', color:'black'}} href='/pages/librarian/reservedashboard/add'>Add</Link></button>
