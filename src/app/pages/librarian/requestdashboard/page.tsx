...
const variants = {
  ...
  const getAllRequest = () =>{
  axios.get("http://localhost:8080/api/request").then((res) =>{
    setrequest(res.data);
  })
  }
  useEffect(() => {
  ...
  const deleteRequest = (id:number) =>{
    axios.delete(`http://localhost:8080/api/request/${id}`).then(() =>{
      setrequest(request.filter(requests => requests.id !== id))
         Swal.fire({
  ...
  return (
    ...
    <div className='logo'><span style={{fontSize:50,color:'orange', fontWeight:'bold'}}>L</span><span style={{fontSize:40,color:'white', fontWeight:'bold'}}>i</span><span style={{fontSize:35,color:'orange', fontWeight:'bold'}}>b</span><span style={{fontSize:25, color:'white', fontWeight:'bold'}}>rary</span></div>
      ...
      transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }}>
              <h1 style={{marginTop:30}} className='text-center text-3xl text-blue-950 font-bold'>The Renewal Interface</h1>
              <br />
              <button><Link style={{textDecoration:'none', color:'black'}} href='/pages/librarian/requestdashboard/add'>Add</Link></button>
                 ...
                 {request.map((r) =>(
                  <tr className='text-center' key={r.id}>
                    ...
                    <td>
                      ...
                      <button onClick={() => deleteRequest(r.id)}  style={{padding:5, width:'70px', marginLeft:5, backgroundColor:' var(--color-red-800)', color:'white'}}>Delete</button> 
                    </td>
                  </tr>
                 ))}
                </tbody>
