...
const page = () => {
 ...
 const router = useRouter();
  ...
  const handleSubmit = ()=>{
    if(!addBorrow.name || !addBorrow.author || !addBorrow.borrowdate){
        Swal.fire({
     title: 'Error!',
      text: 'Fill in to borrow a book.',
      icon: 'error',
      ...
      axios.post("http://localhost:8080/api/borrow", addBorrow).then((res) =>{
        // setaddBorrow({image:"", name:"", author:""});
        setaddBorrow(res.data)
             Swal.fire({
      title: 'Book borrowed successful!',
      text: 'You have successfully borrow a book.',
      icon: 'success',
      confirmButtonText: 'OK',
        confirmButtonColor:'darkblue'
        })
  ...
  return (
   ...
   <div className='overall-borrow flex flex-wrap justify-between'>
      ...
      transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }} >
            <h1>Borrow a Book</h1>
            <br />
     <input type='text' value={addBorrow.name} placeholder='Enter Book_Name' onChange={(e) => setaddBorrow({...addBorrow, name: e.target.value})} />
          <br />
            <br />
          <input type='text' value={addBorrow.author} placeholder='Enter Author_Name' onChange={(e) => setaddBorrow({...addBorrow, author: e.target.value})} />
          <br />
            <br />
          <input type='date' value={addBorrow.borrowdate} placeholder='Enter Date' onChange={(e) => setaddBorrow({...addBorrow, borrowdate:e.target.value})} />
          <br />
            <br />
