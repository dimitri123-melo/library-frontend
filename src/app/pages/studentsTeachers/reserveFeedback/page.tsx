...
const page = () => {
 ...
 const router = useRouter();
  ...
  const [ReserveFeedback, setReserveFeedback] = useState<ReserveFeedback[]>([])

  const getAllReserveFeedback = () =>{
    axios.get("http://localhost:8080/api/reserve").then((res) =>{
        setReserveFeedback(res.data)
    })
  }

  useEffect(() =>{
    getAllReserveFeedback();
  },[])

//    const deleteReserveFeedback = (id:number) =>{
//     axios.delete(`http://localhost:8080/api/borrow/${id}`).then(() =>{
//       setReserveFeedback(ReserveFeedback.filter(loanH => loanH.id !== id));
//          Swal.fire({
...
//    }
  ...
  return (
   ...
   <div className='overall-borrow flex flex-wrap justify-between'>
            <br />
            <motion.div style={{ width:'calc(100% - 20px)'}} className='ReserveFeedback'
      ref={ref}
      initial={{ x: -100, opacity: 0 }}
      animate={inView ? { x: 0, opacity: 1 } : {}}
      transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }} >
            <h1 style={{marginTop:50}} className='text-center text-3xl text-blue-950 font-bold'>The Reserves Feedback</h1>
            <br />
