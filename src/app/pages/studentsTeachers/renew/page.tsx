...
const page = () => {
 ...
 const router = useRouter();
  ...
  const handleSubmit = ()=>{
    if(!addRenew.name || !addRenew.author || !addRenew.renewdate){
        Swal.fire({
     title: 'Error!',
      ...
      axios.post("http://localhost:8080/api/renewBook", addRenew).then((res) =>{
        setaddRenew(res.data)
             Swal.fire({
      title: 'Book renewed successful!',
      text: 'You have successfully renewed a book.',
      icon: 'success',
      confirmButtonText: 'OK',
        confirmButtonColor:'darkblue'
        })
  ...
  return (
   ...
   <div className='overall-borrow flex flex-wrap justify-between'>
      ...
      transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }} >
            <h1>Renew a Book</h1>
            <br />
     <input type='text' value={addRenew.name} placeholder='Enter Book_Name' onChange={(e) => setaddRenew({...addRenew, name: e.target.value})} />
          <br />
            <br />
          <input type='text' value={addRenew.author} placeholder='Enter Author_Name' onChange={(e) => setaddRenew({...addRenew, author: e.target.value})} />
          <br />
            <br />
          <input type='date' value={addRenew.renewdate} placeholder='Enter Date' onChange={(e) => setaddRenew({...addRenew, renewdate:e.target.value})} />
          <br />
            <br />
